...
# Стадии, которые действительно обращаются к серверу инференса. Для остальных
# совет «поднимите параллелизм запросов к модели» бессмысленен: MinerU и
# эмбеддер к языковой модели не обращаются вовсе, и простой карты у них имеет
# совсем другую природу.
LLM_STAGES = frozenset({"enrich", "graph"})

_WAITING_ADVICE: dict[str, str] = {
    "parse": (
        "MinerU чередует работу на карте с разбором разметки на процессоре в один "
        "поток, поэтому карта простаивает по устройству стадии. Ускорение даёт "
        "не параллелизм запросов, а разбор нескольких документов одновременно "
        "либо смена backend на vlm."
    ),
    "embed": (
        "Эмбеддер простаивает между батчами. Поднимите INDEXING_EMBED_WINDOW: "
        "стадия короткая, и накладные расходы на батч заметны на её фоне."
    ),
    "chunk": (
        "Чанкинг идёт на процессоре в один поток; карта здесь не при чём. "
        "Если стадия заметна по времени, смотрите обогащение иллюстраций."
    ),
}
...
def _waiting_advice(stage: str) -> str:
    if stage in LLM_STAGES:
        return _WAITING_ADVICE_LLM
    return _WAITING_ADVICE.get(
        stage,
        "Ресурсы свободны, а стадия идёт. Стадия не обращается к серверу "
        "инференса, поэтому ищите ожидание ввода-вывода или однопоточный участок.",
    )
