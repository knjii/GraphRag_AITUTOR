...
class GraphSettings(_Base):
    ...
    * поиск seed-сущностей идёт через full-text индекс, а не ``CONTAINS`` с полным сканом.
    ...
    # Сколько раз повторить извлечение, прежде чем откатиться к правилам.
    #
    # Разбор неудачных фрагментов показал, что сбой почти всегда разовый: они
    # неотличимы от успешных по длине, насыщенности формулами и языку
    # и разбросаны по всему корпусу без группировки. Движок с непрерывным
    # батчингом не побитово воспроизводим даже при temperature=0, поэтому
    # повтор — осмысленное лекарство, а не суеверие.
    #
    ...
    #
    # Порог задаёт, сколько фрагментов достижимо обходом: при 64 медианный
    # охват — 461 фрагмент из 1151.
    #
    # Значение 40 было подобрано офлайн-замером второго шага (MRR +0.069
    # на отложенной половине пар) и **отклонено проверкой на продукте**:
    # recall −0.012, у связывающих вопросов −0.029, четыре вопроса хуже
    # и ни одного лучше, интервал не покрывает ноль.
    #
    # Причина расхождения важнее самого числа. Отсечение хабов меняет не только
    # обход, но и поиск стартовых сущностей: удалённый узел перестаёт находиться
    # полнотекстовым поиском по тексту вопроса. Офлайн-замер этой части
    # конвейера не видит, поэтому его выводы о пороге неполны по построению.
    ...
    # Во сколько раз вес сущности падает с каждым шагом обхода.
    #
    # Прежде затухание было жёстко зашито как 1/(1+расстояние), то есть 0.5
    # на первом шаге. Замер второго шага показал, что это слишком строго:
    # сосед несёт почти столько же, сколько сама сущность фрагмента, и 0.8
    # даёт MRR 0.253 против 0.225. Нулевое значение выключает расширение —
    # оно тоже проверено и оказалось заметно хуже (доля попаданий 0.32 против 0.50).
    hop_decay: float = Field(default=0.8, ge=0.0, le=1.0, alias="GRAPH_HOP_DECAY")
    # Взвешивать вклад сущности её редкостью.
    #
    # Без этого «векторное пространство» из середины книги весит столько же,
    # сколько «разложение Холецкого», встреченное дважды. Замер: доля попаданий
    # второго фрагмента в первую восьмёрку 0.404 против 0.449 при прочих равных.
    passage_idf_enabled: bool = Field(default=True, alias="GRAPH_PASSAGE_IDF_ENABLED")
...
class RetrievalSettings(_Base):
    ...
    #
    # Оставлена эвристика, и это результат проверки, а не инерция.
    #
    # Модельный роутер сперва показал recall 0.859 → 0.880 значимо. Но он
    # отправляет в граф 100% вопросов, а маршрут в граф заодно расширяет выдачу
    # с top_k до top_k_linking. Разделение эффектов на одном наборе:
    #
    #   эвристика, выдача 8/16 → эвристика, выдача 16 всем:  +0.021, значимо
    #   эвристика с выдачей 16 → «всегда в граф» с выдачей 16: +0.000, 0/0
    #
    # То есть весь прирост дала выдача, а маршрутизация — ровно ничего.
    # Причём и сам «прирост выдачи» отчасти определение: recall@16 по списку
    # из 8 элементов ограничен сверху. На равном k (recall@8 = 0.819)
    # все три конфигурации совпадают.
    #
    # Модельный роутер стоит вызова на каждый вопрос и не даёт ничего.
    ...
    # Сколько мест итогового контекста обязано достаться графовым фрагментам.
    #
    # Ноль — по измерению, а не по умолчанию. Настройка работоспособна
    # (до 2026-08-17 была молчаливым no-op: реранкер возвращал ровно top_k,
    # и подставлять было неоткуда), но не нужна: на маршрутизированных вопросах
    # граф и так занимает медианно шесть мест контекста, и квота в три
    # сработала бы лишь на трёх вопросах из семидесяти.
