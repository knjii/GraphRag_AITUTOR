...
_LEAKY_PATTERNS = (
    "в тексте",
    "в фрагмент",
    # «В данном фрагменте» шаблон «в фрагмент» не ловил (задача 020).
    "данном фрагмент",
    "этом фрагмент",
    "в данном отрывке",
    "согласно отрывку",
...
_NUMBERED = re.compile(
    r"\(\d+\.\d+[а-яa-z]?\)"
    r"|формул\w*\s*\(?\s*\d"
    # У уравнения — только номер вида «(2.3)» или «2.3»: «уравнение 2x + 3 = 0»
    # задаёт само уравнение, а не ссылку (задача 020).
    r"|уравнени\w*\s*(?:№\s*)?(?:\(\s*\d|\d+\.\d)"
    r"|№\s*\d"
    r"|рис\w*\.?\s*\d"
    r"|табл\w*\.?\s*\d"
    r"|раздел\w*\s*\d"
